  ...
  return (
    <AppLayout filters={filters} onFiltersChange={setFilters}>
      <div className="max-w-3xl mx-auto">
        <button onClick={() => navigate("/")} className="inline-flex items-center gap-2 text-primary font-semibold mb-6 hover:underline">
          <ArrowLeft className="h-4 w-4" /> Back
        </button>

        <article className="bg-card rounded-lg border shadow-sm p-8">
          {isEditing ? (
            <input value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className={`${inputCls} text-2xl font-bold mb-4`} />
          ) : (
          ...
          <div className="mb-8">
            ...
            {isEditing ? (
              <input value={editTags} onChange={(e) => setEditTags(e.target.value)} placeholder="tag1, tag2, tag3" className={inputCls} />
            ) : (
